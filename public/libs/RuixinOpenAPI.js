...
var RXShareMenuEnum = {
  ...
  ShareSinaMenu: 			1 << 1,
  ShareWeixinMenu: 			1 << 2,
  ShareWeixinFavoriteMenu: 	1 << 3,
  ShareWeixinCircleMenu: 	1 << 4,
  AllShareMenus:            (1 << 5) - 1
};
...
RuixinApi.prototype.callNative = function(funcName, params) {
	if (window.webkit)
	{
		var fn;
		try {
			fn = eval("window.webkit.messageHandlers." + funcName);
		} catch (e) {
			console.log(funcName + '方法不存在！');
   		}
   		fn.postMessage(params);
	} else if (window.AndroidWebView) {
		var func = 'window.AndroidWebView.' + funcName + '(' + JSON.stringify(params) + ')';
...
/**
 *    统计用户行为
 * @param action 必填， string 类型。用户行为描述，例如 :EnterApprovalDetailsPage 
 * @param extend_json 选填， string 类型。该字段为一个 json 字符串，用户统计扩展信息
 */
RuixinApi.prototype.actionStatistic = function(params) {
	this.callNative("actionStatistic", params);
};

/**
 *    识别二维码
 * @param qrCode Blob ：必填， string 类型。二维码图片base64字符串。
 * @param isDirect: 是否直接到内置浏览器打开， 1 ：直接跳转， 0 ：默认处理。默认处理会按照瑞信 Command 标准进行内部分发操作。
 * @param extend_json 选填， string 类型。该字段为一个 json 字符串，用户统计扩展信息
 */
RuixinApi.prototype.recognizeQRCode = function(params) {
	this.callNative("recognizeQRCode", params);
};
...
/**
 * 添加分享菜单
 * @param menuType 数字类型，RXShareMenuEnum中定义了可以增加为菜单的类型
 * @param params 包含以下参数
 * @param title 必填，收到的用户在分享消息中看到的标题
 * @param summary 必填，收到的用户在分享消息中看到的内容文字
 * @param url 必填，收到的用户点击分享消息后跳转的url
 ...
 */
RuixinApi.prototype.addShareMenus =  function(menuType, params) {
	var type = menuType & RXShareMenuEnum.AllShareMenus;
	var interactiveName = "";
	if ((type & RXShareMenuEnum.ShareQQMenu) > 0)
	{
		interactiveName += ",shareQQ";
	}
	if ((type & RXShareMenuEnum.ShareSinaMenu) > 0)
	{
		interactiveName += ",shareSina";
	}
	if ((type & RXShareMenuEnum.ShareWeixinMenu) > 0)
	{
		interactiveName += ",shareWeixin";
	}
	if ((type & RXShareMenuEnum.ShareWeixinCircleMenu) > 0)
	{
		interactiveName += ",shareWeixinCircle";
	}
	if ((type & RXShareMenuEnum.ShareWeixinFavoriteMenu) > 0)
	{
		interactiveName += ",shareWeixinFavorite";
	}
	// alert(interactiveName.substring(1));
	if (interactiveName.length > 0)
	{
	    params.interactiveName = interactiveName.substring(1);
	    params.interactiveID = "";
	    params.interactiveDetailID = "";
		this.callNative("addInteractiveMenu", params);
	}
...
RuixinApi.prototype.addQQMenu =  function(params) {
	this.addShareMenus(RXShareMenuEnum.ShareQQMenu, params);
};
...
RuixinApi.prototype.addSinaMenu =  function(params) {
	this.addShareMenus(RXShareMenuEnum.ShareSinaMenu, params);
};
...
RuixinApi.prototype.addWeixinFavoriteMenu =  function(params) {
	this.addShareMenus(RXShareMenuEnum.ShareWeixinFavoriteMenu, params);
};
...
RuixinApi.prototype.addWeixinCircleMenu =  function(params) {
	console.log(params)
	this.addShareMenus(RXShareMenuEnum.ShareWeixinCircleMenu, params);
};
...
RuixinApi.prototype.addWeixinMenu =  function(params) {
	this.addShareMenus(RXShareMenuEnum.ShareWeixinMenu, params);
};
...
/**
 * 获取获取用户ip、mac、uuid等信息
 * @param success 成功时的回调函数名称
 * @param fail 失败时的回调函数名称
 */
RuixinApi.prototype.getIpconfig =  function(params) {
	this.callNative("getIpconfig", params);
};
 ...
 * 忽略https证书校验
 */
RuixinApi.prototype.setAllowCertificate =  function(params) {
    this.callNative("setAllowCertificate", params);
};

/**
 *  控制h5页面缓存策略开关
 */
RuixinApi.prototype.setWebviewCacheStrategy =  function(params) {
    this.callNative("setWebviewCacheStrategy", params);
};
 ...
 * @param success 成功回调函数名称
 */
RuixinApi.prototype.ccbMorePay =  function(params) {
    this.callNative("ccbMorePay", params);
};
 ...
 * @param success 成功回调函数名称
 */
RuixinApi.prototype.wechatPay =  function(params) {
    this.callNative("wechatPay", params);
};


/**
 *跳转掌阅JS接口
 * @param zyReaderScheme 回调函数名
 * @param success 成功回调函数名称
 */
RuixinApi.prototype.zyReader =  function(params) {
    this.callNative("zyReader", params);
};
 ...
 * @param success 成功回调函数名称
 */
RuixinApi.prototype.returnToLogin =  function(params) {
    this.callNative("returnToLogin", params);
};
 ...
 * @param success 成功回调函数名称
 */
RuixinApi.prototype.getWifiInfo =  function(params) {
    this.callNative("getWifiInfo", params);
};
